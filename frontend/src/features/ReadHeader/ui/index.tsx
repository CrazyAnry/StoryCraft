  ...
  useEffect(() => {
    const isLiked = async () => {
      if (oneStory) {
        ...
        if(!proggresRes){
          startAndUpdateStory(oneStory.id!, currentUser!.id, oneStory.scenes[0].id)
        }
        setProggresScene(getStoryProgress(oneStory.id!, currentUser!.id)?.currentSceneId)
      }
    }
  ...
  return (
    <div className={s.container}>
      <div className={s.titleRow}>
        ...
        <h2 className={s.scenesCount}>Количество сцен: {oneStory.scenes.length}</h2>
      </div>
      <div className={s.imgContainer}>
        <img className={s.img} src={oneStory.image ? oneStory.image : "/NoImg.png"} />
      ...
      <div className={s.btnContainer}>
        <Link href={`/read/${oneStory!.id}/${proggresScene}`}><button className={s.btn}>Читать</button></Link>
      </div>
    </div>
  );
};
