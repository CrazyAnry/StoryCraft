...
export const follow = async (followerId: number) => {
  try {
    const response = await axiosInstance.post(API_ROUTES.follow.follow + followerId);
...
export const unfollow = async (followerId: number) => {
  try {
    const response = await axiosInstance.delete(API_ROUTES.follow.unfollow + followerId);
    return response.data;
  } catch (error: any) {
    if (axios.isAxiosError(error)) {
