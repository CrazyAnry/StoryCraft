...
> = (set, get) => ({
  ...
  // Actions
  setAllFollows: (allFollows) => set((state) => {
    state.allFollows = allFollows;
  }),
  addFollow: (follow: IFollow) => set((state) => {
    state.allFollows.push(follow);
  }),
  removeFollow: (followingId: number, followerId: number) => set((state) => {
    state.allFollows = state.allFollows.filter((s) => s.followingId !== followingId || s.followerId !== followerId);
  }),
